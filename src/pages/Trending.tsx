...
export default class Trending extends Component<{}, TrendingState> {
  constructor(props: {}) {
    super(props);
    this.state = {
      movies: [],
    ...
    return (
      <div className="pb-32 pt-20 max-w-6xl mx-auto flex flex-col min-h-screen">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 px-4 md:px-0">
          <div>
            <h1 className="text-4xl md:text-5xl font-black text-white italic tracking-tighter uppercase">{t('trending_title')}</h1>
            <p className="text-text-muted text-xs font-bold uppercase tracking-widest mt-1">{t('trending_subtitle')}</p>
          </div>
        ...
        <div className="flex-1 px-4 md:px-0">
          {loading ? (
            ...
            <div className="text-center py-20 bg-surface rounded-3xl border border-surface-high/30 flex flex-col items-center gap-4">
              ...
              <h3 className="text-xl font-bold text-white">{t('trending_empty')}</h3>
              <p className="text-text-muted max-w-sm text-sm">{t('trending_empty_desc')}</p>
            </div>
          )}
        </div>
      </div>
